...
class Download:
    ...
    def __download_file(self, url_obj, file_obj, call_back=None):
        ...
        if self.fileSize:
            with alive_bar(math.ceil(self.fileSize/block_size)) as bar:
                while 1:
                    if self.rate_limit_bucket:
                        if not self.rate_limit_bucket.spend(block_size):
                            sleep(1)
                            continue
                    try:
                        data = url_obj.read(block_size)
                        bar()
    ...
    def download(self, call_back=None, destPathPrefix=None):
        """starts the file download"""
        self.cur_retry = 0
        self.cur = 0
        if destPathPrefix == None:
            destPath = self.download_path
        else:
            destPath = f"{destPathPrefix}/{self.download_path}"
        with open(destPath, "wb") as f:
            if self.auth:
                ...
                urllib2_obj = urllib.request.urlopen(
                    self.url, timeout=self.timeout)
                self.__download_file(urllib2_obj, f, call_back=call_back)
            return self.download_path, True
